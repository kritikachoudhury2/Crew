        ...
        <div className="absolute top-4 right-4">
          ...
          {showMenu && (
            <div className="absolute right-0 mt-2 w-48 rounded-[12px] overflow-hidden shadow-lg z-10" style={{ background: '#2A1A45', border: '1px solid rgba(74,61,143,0.30)' }}>
              <button onClick={() => handleReport('Fake profile')} className="w-full text-left px-4 py-3 font-inter text-sm hover:bg-white/5 flex items-center gap-2" style={{ color: 'rgba(255,255,255,0.7)' }} data-testid="report-btn">
                <Flag size={14} /> Report this profile
              </button>
              <button className="w-full text-left px-4 py-3 font-inter text-sm hover:bg-white/5 flex items-center gap-2" style={{ color: 'rgba(255,255,255,0.7)' }} data-testid="block-btn">
                <Ban size={14} /> Block
              </button>
            </div>
          )}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
            {[
              ...
              { label: 'Training Time', val: parseArr(athlete.training_time).join(', ') || '-' },
          ...
          {sports.includes('ironman') && (
            <div className="rounded-[20px] p-6 border mb-6" style={{ background: 'rgba(42,26,69,0.40)', borderColor: 'rgba(74,61,143,0.20)' }}>
              <h3 className="font-inter font-bold text-sm text-white mb-4">Ironman Details</h3>
              {athlete.ironman_race_type && <p className="font-inter text-sm text-white mb-2">{athlete.ironman_race_type}</p>}
              {athlete.ironman_hours && <p className="font-inter text-xs mb-3" style={{ color: 'rgba(255,255,255,0.6)' }}>{athlete.ironman_hours}/week</p>}
              <div className="flex flex-wrap gap-1.5 mb-2">
                {parseArr(athlete.ironman_strong).map(s => <span key={s} className="px-2 py-0.5 rounded-pill text-[10px] font-inter text-green-400" style={{ border: '1px solid rgba(34,197,94,0.30)' }}>{s}</span>)}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {parseArr(athlete.ironman_weak).map(s => <span key={s} className="px-2 py-0.5 rounded-pill text-[10px] font-inter" style={{ border: '1px solid rgba(212,136,10,0.30)', color: '#F0A830' }}>{s}</span>)}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3">
            ...
            <button className="px-5 py-3 rounded-pill transition-colors"
              style={{ border: '2px solid rgba(74,61,143,0.30)', color: 'rgba(255,255,255,0.5)' }} data-testid="save-profile">
              <Heart size={16} />
            </button>
          </div>
