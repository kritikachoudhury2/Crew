...
import { MessageCircle, Check, X, Clock, Heart, Trash2 } from 'lucide-react';
...
const sportBadge = (s) => ({ hyrox: { bg: '#4A3D8F', l: 'HYROX' }, marathon: { bg: '#D4880A', l: 'MARATHON' }, ironman: { bg: '#0F6E56', l: 'IRONMAN' } }[s] || { bg: '#4A3D8F', l: s?.toUpperCase() });
...
export default function MyConnections() {
  ...
  const fetchAll = async () => {
    ...
    // Matches
    const { data: m } = await supabase.from('matches').select('*, user1:profiles!user1_id(*), user2:profiles!user2_id(*)').or(`user1_id.eq.${user.id},user2_id.eq.${user.id}`);
    setMatches((m || []).map(r => r.user1_id === user.id ? { ...r, partner: r.user2 } : { ...r, partner: r.user1 }));

    // Sent
    const { data: s } = await supabase.from('connect_requests').select('*, to_profile:profiles!to_user_id(*)').eq('from_user_id', user.id).eq('status', 'pending');
    setSentReqs(s || []);

    // Received
    const { data: rv } = await supabase.from('connect_requests').select('*, from_profile:profiles!from_user_id(*)').eq('to_user_id', user.id).eq('status', 'pending');
    setRecvReqs(rv || []);

    // Saved
    const { data: sv } = await supabase.from('saved_profiles').select('*, saved:profiles!saved_user_id(*)').eq('user_id', user.id);
    setSaved(sv || []);

    // Daily count
    const { count } = await supabase.from('connect_requests').select('*', { count: 'exact', head: true })
      .eq('from_user_id', user.id).gte('created_at', new Date(Date.now() - 86400000).toISOString());
  ...
  const handleAccept = async (req) => {
    await supabase.from('connect_requests').update({ status: 'accepted' }).eq('id', req.id);
    await supabase.from('matches').insert({ user1_id: req.from_user_id, user2_id: req.to_user_id });
    fetchAll();
  };
            ...
            <div className="space-y-3">
              ...
              {tab === 'sent' && (sentReqs.length > 0 ? sentReqs.map(r => (
                <ProfileCard key={r.id} p={r.to_profile} actions={
                  <button onClick={() => handleWithdraw(r)} className="font-inter text-xs hover:text-amber-brand flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.4)' }} data-testid={`withdraw-${r.id}`}>
