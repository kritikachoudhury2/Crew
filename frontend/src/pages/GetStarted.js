...
export default function GetStarted() {
  ...
  // Safely get the current user ID from either session or user object
  ...
  const upsertProfile = async (data) => {
    const uid = getUserId();
    if (!uid) {
      console.error('[GetStarted] upsertProfile: no user id found');
      toast.error('Session expired. Please sign in again.');
      return false;
    }

    // Serialize arrays to JSON strings for Postgres text columns
    ...
    if (upsertError) {
      console.error('[GetStarted] upsertProfile error:', upsertError.code, upsertError.message, upsertError.details);
  ...
  const handleSendMagicLink = async () => {
    ...
    try {
      ...
      // Store phone so it can be saved when onboarding completes
  ...
  const handleFinalSave = async () => {
    ...
    const finalData = {
      ...
      ironman_race_type: answers.ironman_race_type || null,
      ...
      ironman_hours: answers.ironman_hours || null,
    ...
    if (saveErr) {
      console.error('[GetStarted] Final save error:', saveErr.code, saveErr.message);
      ...
      case 'hyrox-race':
        return (
          <div className="max-w-md mx-auto">
            ...
            <h2 className="font-inter font-[800] text-3xl text-white mb-6">Your Hyrox basics.</h2>
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Target race</label>
                <select
                  value={answers.target_race || ''}
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Race category</label>
                <div className="space-y-2">
                  {[
                    { val: 'open', label: 'Open', desc: 'Standard solo — most popular' },
                    { val: 'pro', label: 'Pro', desc: 'Competitive solo, targeting a fast time' },
                    { val: 'doubles', label: 'Doubles', desc: 'Two people sharing all stations + running' },
                    { val: 'mixed_doubles', label: 'Mixed Doubles', desc: 'One male + one female' },
                  ].map(c => (
      ...
      case 'hyrox-fitness':
        return (
          <div className="max-w-md mx-auto">
            ...
            <h2 className="font-inter font-[800] text-3xl text-white mb-6">Training habits.</h2>
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Fitness level right now</label>
                <div className="space-y-2">
                  {[
                    ...
                    { val: 'elite', label: 'Elite', desc: 'Podium ambitions' },
                  ].map(l => (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => update('hyrox_5k_time', e.target.value)}
              ...
              <div>
                ...
                <ChipSelect
                  ...
                  selected={answers.training_days}
                  multi={false}
              ...
              <div>
                ...
                <ChipSelect options={TRAINING_TIMES} selected={answers.training_time || []} onToggle={v => toggleArray('training_time', v)} />
              </div>
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
      ...
      case 'hyrox-stations':
        return (
          <div className="max-w-md mx-auto">
            ...
            <h2 className="font-inter font-[800] text-3xl text-white mb-2">Stations & partner.</h2>
            <p className="font-inter text-sm mb-6" style={{ color: 'rgba(255,255,255,0.5)' }}>Helps us find someone who challenges where you're strong — or supports where you're not.</p>
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
                  Strongest stations <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>(optional)</span>
                </label>
                <ChipSelect options={HYROX_STATIONS} selected={answers.hyrox_strong || []} onToggle={v => toggleArray('hyrox_strong', v)} />
              </div>
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
                  Stations to improve <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>(optional)</span>
                </label>
                <ChipSelect options={HYROX_STATIONS} selected={answers.hyrox_weak || []} onToggle={v => toggleArray('hyrox_weak', v)} />
              </div>
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Training partner type</label>
                <div className="space-y-2">
                  {[
                    { val: 'Train regularly', label: 'Regular training partner', desc: 'Same programme, consistent sessions' },
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Preferred partner level</label>
                <ChipSelect
                  options={['Same level', 'Better — push me', 'Happy to guide someone newer', 'Open']}
                  selected={answers.partner_level_pref}
                  multi={false}
                  onToggle={v => update('partner_level_pref', v)}
                />
              </div>
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Partner gender</label>
                <ChipSelect
                  options={['No preference', 'Men only', 'Women only']}
                  selected={answers.partner_gender_pref}
                  multi={false}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <ChipSelect
                  ...
                  selected={answers._marathon_exp}
                  multi={false}
                  onToggle={v => {
        ...
        return (
          <div className="max-w-md mx-auto">
            ...
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
                  Easy long-run pace (per km) <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>(optional)</span>
                </label>
                <input
                  type="text"
                  value={answers.marathon_pace || ''}
                  onChange={e => update('marathon_pace', e.target.value)}
                  placeholder="e.g. 5:45"
                  className="w-full px-4 py-3 rounded-[12px] font-inter text-sm text-white placeholder:text-gray-500 outline-none"
                  style={{ background: 'rgba(42,26,69,0.60)', border: '1px solid rgba(74,61,143,0.30)' }}
                />
                <p className="font-inter text-[11px] mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Your comfortable pace, not race pace.</p>
              </div>
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Weekly kilometres</label>
                <ChipSelect
                  options={['Under 20km', '20–40km', '40–60km', '60–80km', '80km+']}
                  selected={answers.marathon_weekly_km}
                  multi={false}
                  onToggle={v => update('marathon_weekly_km', v)}
                />
              </div>
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Running days per week</label>
                <ChipSelect
      ...
      case 'marathon-partner':
        return (
          <div className="max-w-md mx-auto">
            ...
            <h2 className="font-inter font-[800] text-3xl text-white mb-6">Your goal & partner.</h2>
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Main race goal</label>
                <div className="space-y-2">
                  {[
                    { val: 'Just finish', label: 'Just finish', desc: 'Getting to the line is the goal' },
      ...
      case 'ironman-race':
        return (
          <div className="max-w-md mx-auto">
            ...
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Triathlon distance</label>
                <div className="space-y-2">
                  {[
                    { val: 'Sprint', label: 'Sprint', desc: '750m swim · 20km bike · 5km run' },
                    { val: 'Olympic', label: 'Olympic', desc: '1.5km swim · 40km bike · 10km run' },
                    { val: '70.3 Half', label: 'Ironman 70.3 (Half)', desc: '1.9km swim · 90km bike · 21km run' },
                    { val: 'Full Ironman', label: 'Full Ironman', desc: '3.8km swim · 180km bike · 42km run' },
                  ].map(t => (
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
                  Target event <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>(optional)</span>
                </label>
                <select
                  value={answers.target_race || ''}
                  onChange={e => update('target_race', e.target.value)}
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Triathlon experience</label>
                <ChipSelect
                  options={['None — first triathlon', 'Sprint / Olympic done, stepping up', '1–2 Ironman / 70.3', '3+ races']}
                  selected={answers._ironman_exp}
                  multi={false}
                  onToggle={v => {
                    update('_ironman_exp', v);
                    const map = {
                      'None — first triathlon': 'rookie',
                      'Sprint / Olympic done, stepping up': 'intermediate',
                      '1–2 Ironman / 70.3': 'advanced',
                      '3+ races': 'elite',
                    };
                    update('level', map[v]);
                  }}
      ...
      case 'ironman-training':
        return (
          <div className="max-w-md mx-auto">
            ...
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Training hours per week (all disciplines)</label>
                <ChipSelect
                  options={['Under 6 hrs', '6–10 hrs', '10–15 hrs', '15–20 hrs', '20+ hrs']}
                  selected={answers.ironman_hours}
                  multi={false}
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
                  Strongest disciplines <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>(optional)</span>
                </label>
                <ChipSelect
                  options={['Swim', 'Bike', 'Run', 'Transitions']}
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">
                  Disciplines to improve <span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>(optional)</span>
                </label>
                <ChipSelect
                  options={['Swim', 'Bike', 'Run', 'Transitions']}
      ...
      case 'ironman-partner':
        return (
          <div className="max-w-md mx-auto">
            ...
            <h2 className="font-inter font-[800] text-3xl text-white mb-6">Partner type.</h2>
            <div className="space-y-6">
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Training partner you need most</label>
                <div className="space-y-2">
                  {[
                    { val: 'Full training partner', label: 'Full training partner', desc: 'Swim, bike & run sessions' },
                    { val: 'Swim buddy', label: 'Swim buddy', desc: 'Pool or open water sessions' },
                    ...
                    { val: 'Race day company', label: 'Race day company', desc: 'Share the journey on race day' },
              ...
              <div>
                <label className="font-inter text-xs font-medium text-white block mb-2">Preferred partner level</label>
                <ChipSelect
                  options={['Same level', 'More experienced — push me', 'Happy to support someone newer', 'Open']}
                  selected={answers.partner_level_pref}
                  multi={false}
                  onToggle={v => update('partner_level_pref', v)}
                />
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center">
                  <span
                    className="px-3 py-3 rounded-l-[12px] font-inter text-sm"
                    style={{ background: 'rgba(42,26,69,0.80)', border: '1px solid rgba(74,61,143,0.30)', borderRight: 'none', color: 'rgba(255,255,255,0.4)' }}
                  >@</span>
                  <input
                    type="text"
        ...
        return (
          <div className="max-w-md mx-auto text-center">
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => navigate('/find-a-partner')}
              ...
              <button
                onClick={() => {
                  const uid = getUserId();
                  if (!uid) return;
                  const url = `${window.location.origin}/athlete/${uid}`;
