...
export default function EditProfile() {
  ...
  useEffect(() => {
    if (profile) {
      setForm({
        ...
        training_time: parseArr(profile.training_time),
        hyrox_strong: parseArr(profile.hyrox_strong),
        hyrox_weak: parseArr(profile.hyrox_weak),
        ironman_strong: parseArr(profile.ironman_strong),
        ironman_weak: parseArr(profile.ironman_weak),
      });
    }
  }, [profile]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const data = {
      id: user.id, name: form.name, age: form.age ? parseInt(form.age) : null,
      gender: form.gender, city: form.city, area: form.area, bio: form.bio,
      sport: JSON.stringify(form.sport || []),
      level: form.level, target_race: form.target_race,
      ...
      marathon_distance: form.marathon_distance, marathon_pace: form.marathon_pace,
      marathon_weekly_km: form.marathon_weekly_km, marathon_goal: form.marathon_goal,
      ironman_race_type: form.ironman_race_type,
      ironman_strong: JSON.stringify(form.ironman_strong || []),
      ironman_weak: JSON.stringify(form.ironman_weak || []),
      ironman_hours: form.ironman_hours,
      training_days: form.training_days,
      training_time: JSON.stringify(form.training_time || []),
      partner_goal: form.partner_goal, partner_level_pref: form.partner_level_pref,
    ...
    <div data-testid="edit-profile-page">
      <section className="py-14 md:py-24 px-6 md:px-12 min-h-screen" style={{ background: '#1C0A30' }}>
        <div className="max-w-lg mx-auto">
          <h1 className="font-inter font-[800] text-4xl text-white mb-8" style={{ letterSpacing: '-2px' }}>Edit Profile</h1>
